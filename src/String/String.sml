...
struct
    ...
    and charAt (source: string) (index: int) : char =
        if index < 0 then
            charAt source (length source + index)
        else if index >= length (source) then
            raise IndexError (source, index, length (source))
	    else
	        Basis.String.sub (source, index)

    and compare (l: string) (r: string) : order = Basis.String.compare (l, r)
    ...
    and count (source: string) (substring: string) : int =
        let
            ...
            fun doCount (source: string, count: int) : int =
	            case source of
	    	        ""  => count
	    	      | str => case indexOf source substring of
	    	        ~1 => count
     ...
     *)
    and iterate (source: string) (f: char -> unit) : unit =
        let
            val l = length (source)
            val i = ref 0
        in
    ...
    and python2 (source: string) : Word64.word =
        ...
        in
            iterate source (fn c => x := Word32.xorb (Word32.* (magic, !x), charToWord c));
    ...
    and splitN (source: string) (delim: string) (n: int) : string list =
        if n = 0 then
	        ...
	        let
                ...
                fun doSplitWithoutDelim (s: string, i: int, sl: string list): string list =
                    let
                        val (hd, tl) = split (s, i)
	            ...
	            and doSplit (s: string, sl: string list) : string list =
		            if n = List.length (sl) then
                        revAndOptAddCurrent (s, sl)
		            else
                        case indexOf s delim of
		                    ~1 => revAndOptAddCurrent(s, sl)
		                  | 0 => doSplit (substringToEnd s (length delim), "" :: sl)
		                  | i => doSplitWithoutDelim (s, i, sl)
	        in
	            doSplit (source, [])
            end
    ...
    and stripLeft (source: string) (garbage: string) : string =
        if hasPrefix source garbage then
            stripLeft (substringToEnd source (length garbage)) garbage
	    else
            source

    and stripRight (source: string) (garbage: string) : string =
        let val theEnd = length (source) - 1 in
	        if hasSuffix source garbage then
	            stripRight (substring source 0 (theEnd - length (garbage) + 1)) garbage
	        else
	            source
        end
    ...
    and stripAll (source: string) (garbage: string list) : string =
        let
            fun startsOrEndsWithGarbageHelper (source: string, head: string, tail: string list) : bool =
                if hasPrefix source head orelse
                   hasSuffix source head then
                    true
                else
                    startsOrEndsWithGarbage (source, tail)
            
	        and startsOrEndsWithGarbage (source: string, garbage: string list) : bool =
	            case garbage of
	    	        [] => false
	    	      | hd :: tl => startsOrEndsWithGarbageHelper (source, hd, tl)
